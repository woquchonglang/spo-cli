...
void Ui::renderLogin(moodycamel::ConcurrentQueue<SPOCLI::Event> &queue, std::shared_ptr<SpotifyData> spotifyData) {
    ...
    std::print("width:{},height:{}", Terminal::Size().dimx, Terminal::Size().dimy);
...
void Ui::render(moodycamel::ConcurrentQueue<SPOCLI::Event> &queue, std::shared_ptr<SpotifyData> spotifyData) {
    ...
    // auto userImage = Renderer([&] {
    //     if (spotifyData->userProfile.read().value.has_value()) {
    //         return image_view(spotifyData->userProfile.read().value->image.width,
    //                           spotifyData->userProfile.read().value->image.width,
    //                           spotifyData->userProfile.read().value->image.url);
    //     } else {
    //         return text("Loading...");
    //     }
    // });

    // auto user_home = Container::Vertical({ Container::Vertical({ user_Profile, userImage }), userTopTracksMenu });
